...
print("Num GPUs Available: ", len(get_available_gpus()))
...
if ARGS.use_gpu:
    gpu_memory = [int(x.split()[2]) for x in subprocess.Popen("nvidia-smi -q -d Memory | grep -A4 GPU | grep Free", shell=True, stdout=subprocess.PIPE).stdout.readlines()]
    if gpu_memory:
        os.environ["CUDA_VISIBLE_DEVICES"] = str(np.argmax(gpu_memory))
    else:
...
with tf.variable_scope(tf.get_variable_scope()):
    ...
    # 增加感知损失的权重
    lossDict["percep_t"] = 0.2 * compute_percep_loss(target, transmission_layer, reuse=False)     
    lossDict["percep_r"] = 0.2 * compute_percep_loss(reflection, reflection_layer, reuse=True) 
...
# 修改学习率为 5e-5
opt = tf.train.AdamOptimizer(learning_rate=0.00005).minimize(lossDict["total"], var_list=all_vars)
...
def load_paired_data(img_names, id):
    ...
    if h > max_size or w > max_size:
        ...
        tmp_pureflash = cv2.resize(tmp_pureflash[0], (0, 0), fx=scale_factor, fy=scale_factor)[None, ...]
        tmp_ambient = cv2.resize(tmp_ambient[0], (0, 0), fx=scale_factor, fy=scale_factor)[None, ...]
        tmp_flash = cv2.resize(tmp_flash[0], (0, 0), fx=scale_factor, fy=scale_factor)[None, ...]
        tmp_T = cv2.resize(tmp_T[0], (0, 0), fx=scale_factor, fy=scale_factor)[None, ...]
        tmp_R = cv2.resize(tmp_R[0], (0, 0), fx=scale_factor, fy=scale_factor)[None, ...]
